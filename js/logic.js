...
export function calculateWeightedAverageCost(productId) {
    ...
    if (totalQuantity === 0) return state.REF_DATA.products.find(p => p.id === productId)?.cost || 0; 
...
export function generateIncomeStatement(startDate, endDate) {
    ...
    return { revenue, cogs, grossProfit, payrollExpense, operatingExpenses, depreciationExpense, totalOperatingExpenses, netProfit };
}
...
export function generateBalanceSheet() {
    ...
    for (const code in fullBalances) {
        ...
        if (codePrefix === 1) { 
            ...
            assets.details.push({ code, name: code, balance });
        } else if (codePrefix === 2) {
             liabilities.total += Math.abs(balance);
             liabilities.details.push({ code, name: code, balance: Math.abs(balance) });
        } else if (codePrefix === 3) {
            if (code === '3000') {
                 equity.total += Math.abs(balance);
                 equity.details.push({ code, name: code, balance: Math.abs(balance) });
            } else if (code === '3100') {
                 equity.total -= balance; 
                 equity.details.push({ code, name: code, balance: balance * -1 }); 
            }
        }
    }
    equity.total += totalNetProfitProxy;
    equity.details.push({ code: '3999', name: 'Net Profit (YTD)', balance: totalNetProfitProxy });
